...
log_bill_model_simple <- lm(culmen_depth_mm ~ species + log_length, penguins_data_clean)
plot(log_bill_model_simple, which=2)
plot(log_bill_model_simple, which=1)
summary(log_bill_model_simple)
anova(log_bill_model_simple)

plot_simple <- ggplot(penguins_data_clean, aes(x=culmen_length_mm, y=culmen_depth_mm, color = species)) + geom_point() + geom_abline(intercept = -13.4550, slope = 20.0296) + geom_abline(intercept = -15.3835, slope = 20.0296) + geom_abline(intercept = -18.5845, slope = 20.0296) + scale_colour_manual(values = species_colours) + xlab("Culmen length (mm)") + ylab("Culmen depth (mm)") + theme_bw()
