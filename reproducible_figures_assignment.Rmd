...
bill_size_depth_exploratory <- ggplot(data=chinstrap_data, 
                         ...
                         scale_colour_manual(values = sex_colours) + 
                         scale_shape_manual(values = c(16, 17)) +
                        labs(title = "A graph to show the relationship between culmen depth and culmen length, 
                             and how penguin sex affects this", x= "Culmen Depth (mm)", y="Culmen Length (mm)") +
                         theme_bw()
...
A two-sample t-test will also be conducted to assess whether sex affects culmen depth as well as culmen length. 
Null hypothesis for t-test: There is no difference in mean culmen depth between male and female chinstrap penguins.
Alternative hypothesis for t-test: There is a statistically significant difference in mean culmen depth between male and female chinstrap penguins.
...
#Conducting a t-test analysis to assess whether culmen depth is affected by sex.
t.test(culmen_depth_mm ~ sex, data = chinstrap_data)
```
...
The adjusted R-squared for the model including the interaction is 0.4918, therefore 49.18% of variance is explained by the model. Sex has a significant effect on culmen length (p=<6.43*10^-11), and as such the y-intercepts of the regression lines are significantly different. Culmen depth also has a significant effect on culmen length (p=0.0131). There is no interaction effect between sex and culmen depth (p=0.8927), so the slopes of the regression lines are not significantly different. 
It is possible to reject the following null hypotheses: 
1: There is no effect of sex on culmen length. 
2: There is no effect of culmen depth on culmen length. 

It is not possible to reject the following null hypothesis:
3: There is no interaction effect between sex and culmen depth.

It should be stated that sex does also significantly affect culmen depth, as shown using a two-sample t-test (p=6.6*10^-13).

This therefore shows that sex does have an effect on culmen length in the chinstrap penguins, and that culmen depth also has an independent effect. 
