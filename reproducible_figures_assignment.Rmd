...
bill_size_bodymass_sex_exploratory <- ggplot(data=chinstrap_data, 
                        ...
                        labs(title = "A graph to show the relationship between body mass and culmen length, 
                             and how penguin sex affects this", x= "Body Mass (g)", y="Culmen Length (mm)") +
                         theme_bw()
...
The adjusted R-squared for the model used (including the interaction term) is 0.4803, therefore 48.03% of variance is explained by the model. Sex has a significant effect on culmen length (p=<9.16*10^-11), and as such the y-intercepts of the regression lines are significantly different. Body mass does not seem to have a significant effect on culmen length (p=0.0540). There is no interaction effect between sex and body mass (p=0.2953), so the slopes of the regression lines are not significantly different. 
...
The fact that there is no interaction effect between sex and culmen depth is unsurprising as well, as while both sexes will have differently sized beaks, it is reasonable to assume they would have similar overall beak shapes (a product of the interaction between culmen length and depth) as they are the same species and only display mild dimorphism.

It is unsurprising that sex affects body mass in this species independently, as *P. antarcticus* has been shown to be dimorphic in body mass (Gorman, et al., 2014).
...
In conclusion, it has been shown that culmen length is affected by both body mass and by sex independently, and that there is no interaction effect between body mass and sex in P. antarcticus. These results are unsurprising. This analysis reaffirms the findings of previous research which stated that chinstrap penguins do display sexual dimorphism in their beak morphology. 

Further research should be conducted to clarify why sexual dimorphism has evolved in the chinstrap penguins, and the genus Pygoscelis more widely. Moreover, the trophic ecology of male and female penguins in this genus should be studied to assess whether their trophic ecology differs greatly, and whether this can explain the variation in beak length and so beak morphology or whether some other factor has caused the evolution of sexual dimorphism. 
