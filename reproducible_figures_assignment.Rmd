...
#Making function to allow filtering by species
species_filter <- function(clean_data){
  clean_data %>% 
    filter(species) %>%
  select(species, body_mass_g, clutch_completion) %>%
  remove_NA()}

#Making function to allow filtering by species and by sex
species_sex_filter <- function(clean_data){
  clean_data %>% 
    filter(species, sex) %>%
  select(species, body_mass_g, clutch_completion) %>%
  remove_NA()}

#Testing if new functions work - unsuccessful
adelie_data_test1 <- species_sex_filter(penguins_data_refined)
