...
penguins_data_clean <- cleaning_penguin_columns(penguins_raw)
...
gentoo_data <- penguins_data_clean %>% 
               ...
               remove_NA()
gentoo_data
#Do not need to factorise sex in gentoo data as only 2 groups so no need to run Tukey post-hoc test.
...
## Hypotheses
Effect of species:
Null hypothesis: The mean culmen depth of each species will not be significantly different from that of any other species.
Alternative hypothesis: The mean culmen depth of at least one species will be significantly different from that of at least one other species.

Effect of sex:
Null hypothesis: The mean culmen depth of each sex will not be significantly different from that of any other species.
Alternative hypothesis: The mean culmen depths of each sex will be significantly different from each other.

## Statistical Methods

In this analysis, an ANCOVA test will be conducted, to test whether culmen length and species affect culmen depth, as well as if there is any interaction between the effects of culmen length and species. An ANCOVA is suitable for this analysis as there is a numerical response variable (culmen depth), a categorical explanatory variable (species) and a covariate which must be controlled for (culmen length). It is important to control for culmen length as this may impact the culmen depth on a beak. 

### All species
Null hypotheses for effect of species: 
1: There is no effect of species on culmen depth. 
2: The slope of culmen length is not different from that of culmen depth. 
3: The effect of species on culmen depth is not dependent on the effect of culmen length.

Alternative hypotheses for species effect: 
1: There is a significant effect of species on culmen depth. 
2: The slope of culmen length is significantly different from that of culmen depth. 
3: The effect of species on culmen depth is dependent on the effect of culmen length.

In Gentoo penguins only, an ANCOVA test will also be conducted, testing whether culmen length and species affect culmen depth, as well as if there is any interaction between the effects of culmen length and species.

### Gentoo penguins
Null hypotheses for effect of sex in Gentoos: 
1: There is no effect of sex on culmen depth. 
2: There is no effect of culmen length on culmen depth. 
3: There is no interaction between sex and culmen length.

Alternative hypotheses for sex effect in Gentoos: 
1: There is a significant effect of sex on culmen depth. 
2: There is a significant effect of culmen length on culmen depth. 
3: There is a significant interaction between sex and culmen length.
...
#Running post-hoc Tukey test using glht() function to assess which species have significantly different mean culmen depth
allspecies_int_posthoc <- glht(log_bill_model_int, linfct = mcp(species = "Tukey"))
summary(allspecies_int_posthoc)

#Model without interaction term - necessary as no significant interaction found.
log_bill_model_noint <- lm(culmen_depth_mm ~ species + log_length, penguins_data_clean)
...
#Running post-hoc Tukey test using glht() function to assess which species have significantly different mean culmen depth
allspecies_noint_posthoc <- glht(log_bill_model_noint, linfct = mcp(species = "Tukey"))
summary(allspecies_noint_posthoc)
...
gentoo_log_bill_model_noint <- lm(culmen_depth_mm ~ sex + gentoo_log_length, gentoo_data)
