...
#Making a colour blind-friendly palette useful for distinguishing species on figures. Specifying this ensures the same species are represented by the same colours on any graph where species colours are specified, making this document more accessible (colour blind friendly) and more reproducible.
...
# scale_shape_manual allows specification of shape so different sexes can be designated the      same shapes every time making the figure more reproducible. 
#   Also allows for more in depth exploration of initial data before statistical analysis          conducted.
#Plotted a scatter plot as comparing two numerical variables (culmen length and depth).
...
```{r Gentoo specific}
#Making a colour blind-friendly palette useful for distinguishing sexes on figures. Specifying this ensures each sex is represented by the same colours on any graph where sex-specific colours are needed, making this document more accessible (as it is colour blind friendly) and more reproducible.
